...
// Single Sponsor / Alliance Card Component
export const AllianceCard: React.FC<{ item: AlliancePartner; isPreview?: boolean }> = ({
  item,
  isPreview = false,
}) => {
  ...
  return (
    <div className="group rounded-2xl bg-[#1a1b21] border border-[#2c2e39] hover:border-[#d97d64]/60 transition-all duration-200 p-4 flex flex-col justify-between hover:shadow-xl hover:shadow-[#121316]/50">
      <div className="space-y-3">
        {/* Adaptive Image Container: handles ANY aspect ratio (wide, square, tall) with clean framing */}
        <div className="relative w-full h-24 sm:h-28 rounded-xl bg-[#111215] border border-[#252731] p-3 flex items-center justify-center overflow-hidden transition-colors group-hover:border-[#383b48]">
          {item.imageUrl ? (
            ...
            <div className="flex flex-col items-center justify-center text-[#78746c] text-center p-2">
              <ShieldCheck className="w-5 h-5 text-[#a09c93] mb-1" />
      ...
      <div className="pt-3 mt-3 border-t border-[#262832] flex items-center justify-between gap-2">
...
export const BrandAllianceShowcase: React.FC<BrandAllianceShowcaseProps> = ({
  ...
  // If in preview mode with single or compact display
  if (isPreview) {
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-2">
          <div>
            <div className="inline-flex items-center gap-1 text-[10px] font-semibold text-[#e6cca0] uppercase tracking-wider">
              <Sparkles className="w-2.5 h-2.5 text-[#e6cca0]" />
              <span>Alianzas Culturales</span>
            </div>
            <h4 className="text-xs font-bold text-[#f3f1ec]">
              {title || 'Empresas que Apoyan la Cultura'}
            </h4>
          </div>
        </div>

        <div className={`grid grid-cols-1 ${compact ? 'gap-3' : 'sm:grid-cols-2 gap-3'}`}>
          {alliances.map((item) => (
            <AllianceCard key={item.id || item.name} item={item} isPreview={true} />
          ))}
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      >
        {alliances.map((item) => (
          <AllianceCard key={item.id || item.name} item={item} />
        ))}
      </div>
    </section>
  );
